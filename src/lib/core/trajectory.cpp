...
namespace DynaPlex {
	...
	void Trajectory::SeedRNGProvider(const DynaPlex::System& system, bool eval, int64_t experiment_id, uint32_t secondary_id)
	{
		...
		uint64_t secondary = secondary_id;
		uint64_t combined_id = (secondary_id  * system.WorldSize() + system.WorldRank())*2+eval_int;

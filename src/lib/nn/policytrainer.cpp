...
namespace DynaPlex::NN{
...
#if DP_TORCH_AVAILABLE
    std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> prepare_batch(const std::span<DynaPlex::NN::Sample> samples, const DynaPlex::MDP& mdp) {
        ...
        torch::Tensor mask = torch::full({ batch_size, mdp->NumValidActions() }, 32.0f);
        ...
        return { batched_inputs, batched_targets, mask };
    }
#endif
	...
	void PolicyTrainer::TrainPolicy(DynaPlex::VarGroup nn_architecture, int64_t generation, std::string path_to_sample_data, bool silent) {
        ...
        do{
            ...
            auto [validation_samples, validation_targets, validation_mask] = prepare_batch(validation_data, mdp);
            ...
            float current_validation_loss = validation_loss.item<float>();
